...
contract SupplyChain {
  ...
  */
  ...
  modifier verifyCaller(address _address){ 
    require (_address == msg.sender, "Unauthorized caller");
     _;
  }
  ...
  modifier checkValue(uint _sku) {
    ...
    require(items[_sku].price <= msg.value, "Can't afford that");
    _;
  ...
  modifier forSale(uint _sku){
    require(items[_sku].state == State.ForSale,"Item not marked for sale");
    require(items[_sku].price > 0, "Item not initialized");
    _;
  }
  modifier sold(uint _sku){
    require(items[_sku].state == State.Sold,"Item not marked as sold");
    _;
  }
  modifier shipped(uint _sku){
    require(items[_sku].state == State.Shipped,"Item not marked as shipped");
    _;
  }
  modifier received(uint _sku){
    require(items[_sku].state == State.Received,"Item not marked as received");
    _;
  }
